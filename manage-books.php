                                    ...
                                    <?php foreach ($books as $book): ?>
                                        <tr id="book-<?php echo $book['id']; ?>">
                                            ...
                                            <td>
                                                <?php echo $book['title'];
                                                if ($book['subtitle']) {
                                                    echo '<br><span class="subtitle">' . $book['subtitle'] . '</span>';
                                                } ?>
                                            </td>
                                            <td>
                                                <?php echo $book['id'] ;
                                                    if ($book['douban_id']) {
                                                        echo '<br><span class="douban-id">' . $book['douban_id'] . '</span>';
                                                    }
                                                ?>
                                            </td>
                                            <td>
                                            ...
                                            <td><?php echo $book['publisher'] . '<br>' . $book['pubdate']; ?></td>
                                            <td><?php echo $book['read_date']; ?></td>
                                            <td><?php echo $book['rating']; ?></td>
                                            <td>
                                                <a href="<?php echo $request->makeUriByRequest('id=' . $book['id']); ?>"><?php _e('编辑'); ?></a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="typecho-mini-panel">
                        <form method="post" enctype="multipart/form-data" action="<?php $security->index('/action/books-edit'); ?>">
                            ...
                            <ul class="typecho-option">
                                ...
                                <li>
                                    <label for="read_date" class="typecho-label"><?php _e('阅读日期*'); ?></label>
                                    <input type="date" name="read_date" id="read_date" value="<?php echo date('Y-m-d'); ?>" required>
                                </li>
                                <li>
...
?>
    ...
    <script type="text/javascript">
        (function () {
            $(document).ready(function () {
                ...
                <?php if (isset($request->id)) {
                    $book = $db->fetchRow($db->select()->from($prefix . 'books')->where('id = ?', $request->id));
                ?>
                    $('.typecho-mini-panel').effect('highlight', '#AACB36');
                    document.getElementById('book-id').value = '<?php echo $book["id"] ?>';
                    document.getElementById('action').value = 'update';
                    document.getElementById('title').value = '<?php echo htmlspecialchars($book["title"]); ?>';
                    document.getElementById('author').value = '<?php echo htmlspecialchars($book["author"]); ?>';
                    document.getElementById('publisher').value = '<?php echo htmlspecialchars($book["publisher"]); ?>';
                    document.getElementById('subtitle').value = '<?php echo htmlspecialchars($book["subtitle"]); ?>';
                    document.getElementById('origin_title').value = '<?php echo htmlspecialchars($book["origin_title"]); ?>';
                    document.getElementById('translator').value = '<?php echo htmlspecialchars($book["translator"]);  ?>';
                    document.getElementById('pubdate').value = '<?php echo htmlspecialchars($book["pubdate"]);  ?>';
                    document.getElementById('cover_url').value = '<?php echo htmlspecialchars($book["cover_url"]);  ?>';
                    document.getElementById('preview-image').src = '<?php echo htmlspecialchars($book["cover_url"]);  ?>';
                    document.getElementById('preview-image').style.display = 'block';
                    document.getElementById('douban_id').value = '<?php echo $book["douban_id"] ?>';
                    document.getElementById('read_date').value = '<?php echo $book["read_date"] ?>';
                    document.getElementById('rating').value = '<?php echo $book["rating"] ?>';
                    document.getElementById('description').value = `<?php echo htmlspecialchars($book["description"]);  ?>`;
                <?php } ?>
            });
        })();
        ...
        function fetchDoubanInfo() {
            console.log(document.getElementById('read_date').value);
